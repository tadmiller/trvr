 ...
 *
 * # Description
 * Used to handle inputs with the Oculus hand controllers.
 * 
 * #Source
 * Oculus Integration Assets Package for Unity
 * 
 */
...
public class OVRGearVrControllerTest : MonoBehaviour
{
	...
	void Update()
	{
		...
		try
		{
			...
			if (OVRInput.GetUp(OVRInput.RawButton.A)) //when A button is pressed, the next scene is loaded
			{
				Debug.Log("dil");
			...
			if (OVRInput.GetUp(OVRInput.RawButton.B)) //when B button is pressed, the previous scene is loaded
            {
				Debug.Log("dil2");
			...
			if (OVRInput.GetDown(OVRInput.RawButton.Y)) //when Y button is pressed, player's position is reset using Oculus API method
            {
				Debug.Log("resetting pos");

                UnityEngine.XR.InputTracking.Recenter();
                OVRCameraRig.transform.position = TeleportTarget.transform.position;
            }

			if (OVRInput.GetUp(OVRInput.RawButton.X))  //X button is used to manipuated condition variables to make distinctions between calibration sections of tutorial
                x = false; //when X button is released

			if (OVRInput.GetDown(OVRInput.RawButton.X))
				x = true; //when X buttons is held down

			if (OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, OVRInput.Controller.Touch) > 0.8)
				DEBUG1 = true;
			else
				DEBUG1 = false;

			if (OVRInput.Get(OVRInput.Axis1D.SecondaryHandTrigger, OVRInput.Controller.Touch) > 0.8)
				DEBUG2 = true;
			else
				DEBUG2 = false;
		}
